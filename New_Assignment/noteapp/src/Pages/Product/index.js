...
const Product = () => {
    ...
    const fetchItems = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/user/getAllproductDetail"
        );
  ...
  const deleteCatgoryItem = async (id) => {
    try {
      await axios.delete(
        `http://localhost:5000/api/user/deleteProductById/${id}`
      );
      const response = await axios.get(
  ...
  const filterProduct = productDetails?.filter((el) => {
    return el?.productName?.toLowerCase()?.includes(searchText?.toLowerCase());
  });
